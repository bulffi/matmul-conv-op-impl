...
pybind11::array_t<float> mlu_matmul_multi_core_wrapper(const pybind11::array_t<float>& M, const pybind11::array_t<float>& N) {
    auto m = M.unchecked<2>();
    auto n = N.unchecked<2>();
    std::size_t m_x = m.shape(0);
    std::size_t m_y = m.shape(1);
    ...
    /// ==============================================
    int code = mlu_matmul_multi_core(m.data(0, 0), n.data(0, 0), result, m_x, m_y, n_y);
    /// ===============================================
...
pybind11::array_t<float> mlu_matmul_multi_sram_wrapper(const pybind11::array_t<float>& M, const pybind11::array_t<float>& N) {
    auto m = M.unchecked<2>();
    auto n = N.unchecked<2>();
    std::size_t m_x = m.shape(0);
    std::size_t m_y = m.shape(1);
    ...
    /// ==============================================
    int code = mlu_matmul_multi_sram(m.data(0, 0), n.data(0, 0), result, m_x, m_y, n_y);
    /// ===============================================
...
PYBIND11_MODULE(mlu_op, m){
    ...
    m.def("mlu_matmul_multicore", &mlu_matmul_multi_core_wrapper, "multiply 2 matrix with mlu multi core");
    m.def("mlu_matmul_multi_sram_wrapper", &mlu_matmul_multi_sram_wrapper, "multiply 2 matrix with mlu multi sram");
}